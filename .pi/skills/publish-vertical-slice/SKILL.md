...
description: Publishes the current vertical-slice branch and opens a pull request to the dev branch, returning the PR link without approving or merging. Use when asked to push or publish a vertical-slice branch and create its PR to dev.
---

# Publish Vertical Slice

Push the current vertical-slice branch and open a pull request to `dev`, then report the PR link. The PR is created but never approved, merged, or otherwise acted on — approval/merge is always left to the user.
...
- Review the `--dry-run` output before creating the PR: confirm the title, the Summary, and that the diff stat reflects only the slice's intended changes. If the body needs richer content, pass `--body-file` with a customized body.
- Return the PR link to the user.
- Do **not** approve, merge, or request-review the PR, and do not close or modify the linked issue, unless the user explicitly asks.
